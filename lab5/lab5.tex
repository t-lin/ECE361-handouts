...
\label{sec:exercise3}
%-------------------------------------------------------------------------------------------------
To connect multiple LAN and create a larger network, we use routers. We replace one of the bridges in part 2 with a router (named as \texttt{r4}) and create three different LANs, as shown in Fig.~\ref{part-3-router-1}. 

\figvs{0.85}{part-3-router-1}{trim=0cm 0cm 0cm 0cm,clip}{Topology for Part 3, Routing}

Each host, as well as each router's interfaces, have their own IP addresses. A host can detect if the destination host is in the same VLAN by comparing their IPs and netmasks. 
If it detects the destination host does not belong to the same LAN, it merely creates the frame with its source IP and MAC addresses and the destination IP of the target host but with the destination MAC address of the router interface which connects to its LAN. Each router's interfaces act like a host that is connected to the LAN. The router then looks into its routing table. 

A routing table determines which router interfaces should be used to send the frame out to the target host. The entries in the routing table can be inserted manually or dynamically using routing algorithms. Then, the router creates another frame with the same source and destination IP of the received frame, but with its MAC address as the source MAC address and the next-hop MAC address as the destination MAC address of the frame. 

In Fig.~\ref{part-3-router-1}, assume Host 1 wants to send a frame to Host 8. It detects Host 8 is in another network. Thus, it constructs a frame with its source IP and MAC addresses and IP address of Host 8. But it puts the MAC address of the router's interface, which is connected to its LAN (i.e. r4-eth1) as the destination MAC address. The bridge \texttt{s1} then forwards the frame to the router.

When the frame arrives at the router, the router looks into its routing table and search for an entry which includes the destination IP address of the frame. Using the routing table, the right interface is selected to send out the frame. 

%-------------------------------------------------------------------------------------------------
\subsection{Task}
\label{subsec:routing}
%-------------------------------------------------------------------------------------------------
For the last part, you should find out the right source and destination IP and MAC addresses, as well as the interface in which a frame should be sent to when it arrives at the router. In particular, you have to complete these functions:


\begin{itemize}
    \item \textbf{send\_frame\_by\_router(dpid, src\_mac, dst\_mac, src\_ip, dst\_ip)}: This function is called when a frame arrives at the router. The router asks for appropriate source and destination IP and MAC addresses as well as the interfaces the frame should be sent out to. You should return a list containing source MAC, destination MAC, source IP, destination IP and the interface number. The order of these values should be the same as mentioned.

    \item \textbf{get\_next\_hop(dpid, ip\_addr):} Returns the next-hop IP and output port number. In a router, when its interface is directly connected to LAN, the next-hop IP is the router interface IP. 
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Run the controller}: Run (or restart) the controller. Please note, each time you chagne your code, you have to restart the controller.
    \item \textbf{Run the network topology file}: Execute the \texttt{network\_topo\_part3.py} file.
\end{enumerate}
For example, for a comeplete code, you may see the similar results as shown in Listing~\ref{lst:start-mininet3}:
